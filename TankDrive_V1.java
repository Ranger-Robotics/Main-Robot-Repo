...
//starts the controller board
	...
	public void setAllMotors(float value) {
		motor1.set(value);
		motor2.set(value);
		motor3.set(value);
		motor4.set(value);
	...
	public void operatorControl() {
		...
		while (isOperatorControl() && isEnabled()) {
			...
			//The following jumble of if states just grabs the Y value off both joysticks
			...
			if (stick.getRawAxis(5) < -DZ)  {
				...
				if (stick.getY() > DZ)  {
					motor3.set((stick.getY()-DZ));
					motor4.set((stick.getY()-DZ));
				}else
					if (stick.getY() < -DZ){
						motor3.set((stick.getY()+DZ));
						motor4.set((stick.getY()+DZ));
